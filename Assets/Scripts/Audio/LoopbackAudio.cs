...
public class LoopbackAudio : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        PostScaledBeats = new bool[SpectrumSize];
        ...
        _realtimeAudio = new RealtimeAudio(SpectrumSize, ScalingStrategy,(spectrumData) =>
        {
            ...
            for (int i = 0; i < SpectrumSize; i++)
            {
                ...
                totalSpectrumValue += SpectrumData[i];
            ...
            //Beat calculation 
            //if _postScaleAverage is bigger than postscaledenergy, then there is a beat.  
            for (int i = 0; i < PostScaledSpectrumData.Length; i++)
            {
                if (PostScaledSpectrumData[i] > PostScaledEnergy)
                {
                    PostScaledBeats[i] = true;
                }
                else
                {
                    PostScaledBeats[i] = false;
                }
            }
    ...
    public bool[] GetAllBeatData()
    {
        return PostScaledBeats;
    } 

    public bool GetBeatData(int index)
    {
        return PostScaledBeats[index];
    }
